...
router.get('/:zone_cible/:groupe_cible', (req, res, next) => {
    const zone_cible = req.params.zone_cible;
    const groupe_cible = req.params.groupe_cible;
    db.executeSql("select * from  KAPI_notification where zone_cible = '" + zone_cible + "' and groupe_cible = '" + groupe_cible+"'", (error, result) => {
        if (error) throw error;
        res.send(result);
    });
